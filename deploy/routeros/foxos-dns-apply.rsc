...
:local expectedConfirmation ("ADD " . $hostname . " " . $dnsAddress)
...
:if ([:len $records] = 1) do={
  ...
  /ip/dns/static add name=$hostname type=A address=$dnsAddress ttl=5m comment="foxos:dns:admin"
}
:set FoxOSDNSConfirmation ""
:local verified [/ip/dns/static find where name=$hostname address=$dnsAddress type=A comment="foxos:dns:admin"]
:if ([:len $verified] != 1) do={ :error "DNS 写入后回读失败" }
:put ("DNS VERIFIED: " . $hostname . " -> " . $dnsAddress)
