...
:local imageFile ""
:local managementBridge "bridge1"
...
:if ([:len [/container/mounts find where name="foxos-data"]] = 0) do={
  /container/mounts add name=foxos-data src=foxos-data dst=/data
}
:if ([:len [/container/mounts find where name="foxos-backups"]] = 0) do={
  /container/mounts add name=foxos-backups src=foxos-backups dst=/backups
}
...
/container/add file=$imageFile interface=veth-foxos root-dir=containers/foxos envlist=foxos-env mounts=foxos-data,foxos-backups logging=yes start-on-boot=yes comment="foxos:active"
