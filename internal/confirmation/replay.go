...
func NewReplayGuard() *ReplayGuard {
	return &ReplayGuard{used: make(map[[32]byte]time.Time), now: time.Now}
}
func (g *ReplayGuard) Consume(token string) error {
	...
	g.mu.Lock()
	defer g.mu.Unlock()
	for key, usedAt := range g.used {
		if now.Sub(usedAt) > 15*time.Minute {
			delete(g.used, key)
		}
	}
	if _, exists := g.used[digest]; exists {
