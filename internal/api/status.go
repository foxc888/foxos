...
func (s *Server) RegisterStatus(mux *http.ServeMux, ros RouterOSReader, mihomo MihomoReader) {
	mux.Handle("GET /api/v1/routeros/overview", s.auth(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		interfaces, err := ros.Interfaces(ctx)
		if err != nil {
			problem(w, http.StatusServiceUnavailable, "routeros_interfaces", err)
			return
		}
		devices, err := ros.Devices(ctx)
		if err != nil {
			problem(w, http.StatusServiceUnavailable, "routeros_devices", err)
			return
		}
