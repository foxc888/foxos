...
func(s *Server)RegisterBindingPlan(mux *http.ServeMux,reader LeaseReader,signer *confirmation.Signer,executor BindingExecutor,guard *confirmation.ReplayGuard){
	...
	mux.Handle("POST /api/v1/routeros/plans/device-binding/execute",s.auth(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){
		...
		writeJSON(w,http.StatusOK,map[string]any{"status":"applied","policyId":input.Plan.PolicyID})
	})))
}
