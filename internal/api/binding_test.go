...
func TestBindingPreviewReturnsConfirmationAndExecuteConsumesOnce(t *testing.T){
	...
	var preview struct{Plan routeros.Plan `json:"plan"`;ConfirmationToken string `json:"confirmationToken"`}
	if err:=json.Unmarshal(response.Body.Bytes(),&preview);err!=nil{t.Fatal(err)}
	if preview.ConfirmationToken==""||!preview.Plan.RequiresConfirmation{t.Fatalf("preview=%+v",preview)}
	...
	for index,want:=range []int{http.StatusOK,http.StatusConflict}{
		execute:=httptest.NewRequest("POST","/api/v1/routeros/plans/device-binding/execute",strings.NewReader(string(executionBody)));execute.Header.Set("Authorization","Bearer "+token)
