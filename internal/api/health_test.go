...
func TestHealthReadinessReflectsRequiredChecks(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "required path unavailable", requiredPath: filepath.Join(t.TempDir(), "missing"), want: http.StatusServiceUnavailable},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			app.RegisterHealth(mux, HealthOptions{Store: healthStoreStub{err: test.storeErr}, RequiredPaths: []string{test.requiredPath}, Dependencies: []HealthDependency{{Name: "routeros", Configured: true, Check: func(context.Context) error { return test.dependency }}}})
