...
func(e *BindingExecutor)Execute(ctx context.Context,plan Plan,token string)error{
	...
	if !plan.RequiresConfirmation||len(plan.Operations)==0{return ErrUnsafeOperation}
	for _,operation:=range plan.Operations{if err:=validateBindingOperation(operation);err!=nil{return err}}
	for _,operation:=range plan.Operations{if err:=e.writer.Apply(ctx,operation);err!=nil{return fmt.Errorf("RouterOS operation failed: %w",err)}}
	return nil
}
func validateBindingOperation(operation Operation)error{
	if operation.Method!=http.MethodPut&&operation.Method!=http.MethodPatch{return fmt.Errorf("%w: method",ErrUnsafeOperation)}
	if operation.Path!="/rest/ip/dhcp-server/lease"&&!strings.HasPrefix(operation.Path,"/rest/ip/dhcp-server/lease/*"){return fmt.Errorf("%w: path",ErrUnsafeOperation)}
	if !strings.HasPrefix(operation.OwnedComment,"foxos:device:"){return fmt.Errorf("%w: owner",ErrUnsafeOperation)}
	if operation.Body["comment"]!=operation.OwnedComment{return fmt.Errorf("%w: comment mismatch",ErrUnsafeOperation)}
	return nil
}
