...
func TestReadOverviewAndDevices(t *testing.T){
	server:=httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){
		user,password,ok:=r.BasicAuth();if !ok||user!="foxos"||password!="secret"{w.WriteHeader(http.StatusUnauthorized);return}
...
func TestRejectsRedirect(t *testing.T){
	redirect:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){http.Redirect(w,r,"https://example.com",http.StatusFound)}));defer redirect.Close()
	client,err:=NewClient(redirect.URL,"foxos","secret");if err!=nil{t.Fatal(err)}
	if _,err:=client.Resource(context.Background());err==nil{t.Fatal("expected redirect rejection")}
}
