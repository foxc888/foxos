...
func TestClientAppliesOnlyOwnedBindingAndVerifies(t *testing.T){
	...
	server:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){
		user,password,_:=r.BasicAuth();if user!="foxos"||password!="secret"{w.WriteHeader(401);return}
		switch{
		case r.Method==http.MethodPatch&&r.URL.Path=="/rest/ip/dhcp-server/lease/*1":
			var body map[string]string;_ = json.NewDecoder(r.Body).Decode(&body)
			if body["comment"]!="foxos:device:phone"{t.Errorf("body=%+v",body)}
			applied=true;w.WriteHeader(http.StatusOK)
	...
	defer server.Close()
	client,err:=NewClient(server.URL,"foxos","secret");if err!=nil{t.Fatal(err)}
	...
	if err:=client.Apply(context.Background(),operation);err!=nil{t.Fatal(err)}
	if !applied{t.Fatal("operation not applied")}
	if err:=client.Verify(context.Background(),Plan{Operations:[]Operation{operation}});err!=nil{t.Fatal(err)}
}
func TestClientBlocksArbitraryWritePath(t *testing.T){
	client,err:=NewClient("http://127.0.0.1","foxos","secret");if err!=nil{t.Fatal(err)}
	...
	if err==nil{t.Fatal("expected unsafe operation error")}
}
