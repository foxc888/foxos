...
func PlanDeviceBinding(policy domain.DevicePolicy,leases []Lease)(Plan,error){
	if err:=policy.Validate();err!=nil{return Plan{},err}
	...
	for index:=range leases{
		...
		if lease.Address==policy.StaticIP&&leaseMAC!=""&&leaseMAC!=mac{return Plan{},fmt.Errorf("%w: IP %s is already used by %s",ErrPlanConflict,policy.StaticIP,leaseMAC)}
		if leaseMAC==mac{copy:=lease;current=&copy}
	}
	...
	if strings.TrimSpace(current.ID)==""{return Plan{},fmt.Errorf("%w: existing lease has no RouterOS ID",ErrPlanConflict)}
	...
	if current.Address!=policy.StaticIP{plan.Warnings=append(plan.Warnings,fmt.Sprintf("设备地址将从 %s 改为 %s",current.Address,policy.StaticIP))}
	if current.Dynamic=="true"{plan.Warnings=append(plan.Warnings,"现有动态租约将转换为静态租约")}
	return plan,nil
}

func CanonicalMAC(value string)(string,error){
	mac,err:=net.ParseMAC(value);if err!=nil{return "",err}
