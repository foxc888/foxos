...
func TestControllerHealthAndReload(t *testing.T) {
	...
	defer server.Close()
	controller, err := NewController(server.URL, "secret", "/root/.config/mihomo/config.yaml")
	if err != nil {
		t.Fatal(err)
	}
...
func TestControllerValidatesYAMLBeforeReload(t *testing.T) {
	controller, err := NewController("http://127.0.0.1:9090", "", "/config.yaml")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := controller.Validate(context.Background(), path); err == nil {
