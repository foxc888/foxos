...
func TestValidatedRuntimeRejectsBeforeControllerMutation(t *testing.T) {
	...
	_, err := (&Applier{ConfigPath: config, BackupDir: filepath.Join(root, "backups"), Runtime: validated, IntegrityKey: []byte(testApplyIntegrityKey)}).Apply(context.Background(), []byte("mode: broken\n"))
