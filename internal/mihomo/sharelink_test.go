...
func TestParseVLESS(t *testing.T){
	...
	if node.Type!="vless"||node.Name!="HK-01"||node.Port!=443||!node.TLS||node.Path!="/ws"{t.Fatalf("node=%+v",node)}
}
func TestParseTrojan(t *testing.T){
	node,err:=ParseShareLink("trojan://secret@example.com:443?sni=example.com#US-01")
	if err!=nil{t.Fatal(err)}
	if node.Type!="trojan"||node.Password!="secret"||node.SNI!="example.com"{t.Fatalf("node=%+v",node)}
}
